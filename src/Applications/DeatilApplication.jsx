...
useEffect(()=>{
  ...
  const response=await axios.get(`https://backendinternarea.onrender.com/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://backendinternarea.onrender.com/application/${id}`,{action})
